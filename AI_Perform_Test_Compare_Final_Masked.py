...
# 정규식(서버)에서 100% 처리하는 라벨들: AI가 잡아도 predictions에 기록하지 않음
REGEX_HANDLED_LABELS = {
    "PHONE", "EMAIL",
    "PERSONAL_CUSTOMS_ID", "RESIDENT_ID", "PASSPORT", "DRIVER_LICENSE", "FOREIGNER_ID", "BUSINESS_ID", "MILITARY_ID",
    "API_KEY", "GITHUB_PAT", "PRIVATE_KEY",
    "CARD_NUMBER", "IMEI", "CARD_EXPIRY", "BANK_ACCOUNT",
    "HD_WALLET", "PAYMENT_URI_QR",
...
def percentile(arr, p):
    ...
    return float(arr_sorted[f] * (c - k) + arr_sorted[c] * (k - f))
...
def infer_generation(
    ...
    (모델 선정 지표 집계: prompt adherence, json parsed, per-item latency)
    """
    from transformers import AutoTokenizer, AutoModelForCausalLM

    # 통계 누적 (모델 선정 지표)
    n_items = 0
    adhere_cnt = 0     # 시스템 프롬프트 준수(순수 JSON만 출력)
    parsed_cnt = 0     # JSON 파싱 성공
    item_latencies = []  # 항목별 지연시간(초)
    ...
    # fast tokenizer 내부 트렁케이션(보조 안전장치)
    try:
        ...
        with open(out_path, "w", encoding="utf-8") as w:
            for obj in prompts:
                ...
                for e in ents_in:
                    ...
                    # ---------- 여기가 핵심: 정규식에서 이미 처리하는 라벨/마스크 토큰 무시 ----------
                    if action != "dropped":
                        # (선택) 값이 전형적 마스킹 토큰 형태면 무시하고 로그 남김
                        # 예: PHONE, <PHONE>, [EMAIL], {IPV4} 등
                        if re.fullmatch(r'[\[\(\{<]*[A-Z_]+[\]\)\}>]*', value or ""):
                            filter_actions.append({
                                "source_type": orig_type,
                                "normalized": norm_type,
                                "final_type": None,
                                "value": value,
                                "action": "ignored_mask_token"
                            })
                            continue

                        # 정규식에서 100% 처리하는 라벨은 AI 결과를 predictions에 기록하지 않음
                        if final_type in REGEX_HANDLED_LABELS:
                            filter_actions.append({
                                "source_type": orig_type,
                                "normalized": norm_type,
                                "final_type": None,
                                "value": value,
                                "action": "ignored_regex"
                            })
                            continue
                    # ---------------------------------------------------------------------
                    ...
                    filter_actions.append({
                        ...
                        "value": value,
                        "action": action if action=="dropped" else "kept_or_mapped"
                    })
...
def build_pdf_report(out_pdf, summary_list, per_label_union):
    ...
    for s in summary_list:
        ...
        data.append([
            ...
            f"{s['f1_micro']:.3f}",
            f"{s['f1_macro']:.3f}",
            f"{s.get('latency',0.0):.3f}",
            f"{s.get('throughput',0.0):.2f}",
            f"{100.0*ms.get('adherence_rate',0.0):.1f}",
            f"{100.0*ms.get('parsed_rate',0.0):.1f}",
            f"{ms.get('lat_p50',0.0):.3f}",
    ...
    model_select_png = os.path.join(os.path.dirname(out_pdf), "model_select.png")
...
def run_one_model(task, prompts_path, answers_path, model_name, model_id,
    ...
    if task == "token":
        ...
        model_sel_stats = {"n":0,"adhere":0,"parsed":0,"latencies":[]}
    else:
        model_sel_stats = infer_generation(
            prompts_path=prompts_path,
            model_id=model_id,
            out_path=pred_path,
            device=device,
    ...
    n_prompts = len(load_jsonl(prompts_path)) or 1
    latency_avg = (t1 - t0) / n_prompts
    throughput = n_prompts / max(1e-9, (t1 - t0))
    ...
    N = int(ms.get("n", 0))
    adh = int(ms.get("adhere", 0))
    parsed = int(ms.get("parsed", 0))
    lats = list(ms.get("latencies", []))

    metrics["model_select"] = {
        "n": N,
        "adherence": adh,
        "parsed_ok": parsed,
        "adherence_rate": (adh / N) if N else 0.0,
...
def main():
    ...
    print("\n\n📋 [성능 요약표]")
    print(f"{'모델명':<15} {'P_micro':>9} {'R_micro':>9} {'F1_micro':>9} {'F1_macro':>9} {'Latency(s)':>12} {'Throughput':>12}  {'Adh%':>7} {'Parsed%':>8} {'p50(s)':>8} {'p95(s)':>8}")
    print("-"*120)
    for s in summaries:
        ms = s.get("model_select", {})
        adh_pct = 100.0 * ms.get("adherence_rate", 0.0)  # 프롬프트 준수율(%)
        par_pct = 100.0 * ms.get("parsed_rate", 0.0)     # JSON 파싱 성공률(%)
        p50 = ms.get("lat_p50", 0.0)
        p95 = ms.get("lat_p95", 0.0)
        print(
            f"{s['model']:<15} "
            f"{s['precision_micro']:.4f} {s['recall_micro']:.4f} {s['f1_micro']:.4f} {s['f1_macro']:.4f} "
            f"{s['latency']:.3f} {s['throughput']:.2f}  "
            f"{adh_pct:7.1f} {par_pct:8.1f} {p50:8.3f} {p95:8.3f}"
        )
